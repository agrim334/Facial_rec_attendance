<template>
  ...
  <div class='home' v-if='this.attds && this.attds.length !== 0'>
    ...
    <b-table :primary-key='attds.mid'
      :items='attds' :current-page='currentPage' :per-page='perPage'
...
export default {
  ...
  computed: {
    fields() {
      if (this.attds.length !== 0) {
        const ar = Object.keys(this.attds[0]);
        const last = Object.keys(this.attds[0]).length - 1;
